...
describe("CadenceTimer", () => {
  ...
  it("schedules first tick after intervalMs", () => {
    const onTick = vi.fn();
    const t = new CadenceTimer({ intervalMs: 500, onTick, logger: silentLogger() });
    t.start();
  ...
  it("does not overlap: each tick awaits the previous callback", async () => {
    ...
    const t = new CadenceTimer({ intervalMs: 100, onTick, logger: silentLogger() });
    t.start();
...
describe("EventDebouncer", () => {
  ...
  it("trigger() schedules flush after delay", () => {
    const onFlush = vi.fn();
    const d = new EventDebouncer({ delayMs: 50, onFlush, logger: silentLogger() });
    d.trigger();
