...
const runOsascript = async (
  ...
  if (result.exitCode !== 0) {
    throw new Error(`osascript failed: ${result.stderr.trim() || "exit " + result.exitCode}`);
  }
};
...
const readStatus = async (deps: DarwinDeps): Promise<MediaStatus> => {
  const [stateResult, trackResult] = await Promise.all([
    deps.executor.run("osascript", ["-e", SCRIPT_GET_STATE], { timeoutMs: 2_000 }),
    deps.executor.run("osascript", ["-e", SCRIPT_GET_TRACK], { timeoutMs: 2_000 }),
  ]);
