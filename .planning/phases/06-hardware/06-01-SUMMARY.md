...
- `packages/cli/src/util/device-config.ts` — `loadDeviceConfig` + `saveDeviceConfig`. Path: `${xdgConfigHome}/sireno-deck/device.json`. Atomic write via `.tmp` + rename.
...
- **Phantom `@sireno-deck/cli` workspace dep removed**: the emulator's package.json had `"@sireno-deck/cli": "workspace:*"` which doesn't resolve (no such package exists; the alias is via vite config). Removed to unblock `pnpm install`.
...
- `device.json` path: `${xdgConfigHome}/sireno-deck/device.json`. On macOS this is `~/Library/Application Support/sireno-deck/device.json` (resolved via `xdgConfigHome`).
