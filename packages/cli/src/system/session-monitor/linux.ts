...
export const createLinuxSessionProvider = async (
  ...
  try {
    const proxy = await deps.dbus.getProxyObject(SCREENSAVER_SERVICE, SCREENSAVER_PATH);
  ...
  try {
    const idleProxy = await deps.dbus.getProxyObject(IDLE_MONITOR_SERVICE, IDLE_MONITOR_PATH);
    const idleIface = idleProxy.getInterface(IDLE_MONITOR_IFACE);
    await idleIface.GetIdletime?.();
    idleSupported = true;
  } catch (err) {
    deps.logger.debug({ err }, "session: IdleMonitor init failed; idle polling disabled");
  }
