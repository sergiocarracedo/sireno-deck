...
import { describeWeatherCode } from '../domain/codes'
import { fetchWeather } from '../domain/fetch'
import { WeatherButtonSchema, type WeatherSnapshot } from '../schemas'
  ...
  render: ({ config }) => {
      ...
      ? [
          {
            label: 'Temp',
            tone: 'fg' as const,
            value: `${snapshot.temperature ?? '—'}${tempUnit}`,
          },
          {
            label: 'Wind',
            tone: 'fg' as const,
          ...
          },
          {
            ...
            value: describeWeatherCode(
              snapshot.description !== undefined
                ? Number.parseInt(snapshot.description, 10)
                : undefined,
            ),
          },
    ...
    return (
      <div className="h-full w-full">
        <LabelValueListSurface lines={rows} />
      </div>
    )
  },
}
