...
function getRenderedButton(
  runtime: ReturnType<typeof createDeckRuntime>,
  keyIndex: number,
) {
  const renderedButton = runtime
    .getRenderButtons()
    .find((button) => button.keyIndex === keyIndex)
...
const createDisplayDefinition = () => ({
  ...
  }) => createTextSurface(button.position, config.label),
  type: 'display-text',
})

function createNavigationDefinition(
  label: string,
  targetDeckId: string,
  type: string,
) {
...
function createSessionMonitorDouble(
  initialSnapshot: SessionSnapshot,
): SessionMonitor & { emit: (snapshot: SessionSnapshot) => void } {
...
describe('createDeckRuntime', () => {
  ...
  it('renders a bundled addon-backed button through the generic runtime host', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(renderedButton).toMatchObject({
        background: '#10161f',
        keyIndex: 0,
        label: 'Clock',
      })
      ...
      expect(runtimeButton).toMatchObject({
        background: '#10161f',
        keyIndex: 0,
        label: 'Clock',
      })
  ...
  it('preserves ordinary React DOM renders as browser-hosted content', async () => {
    ...
    expect(renderedDeck?.[0]).toMatchObject({
      background: '#10161f',
      keyIndex: 0,
    })
    expect(renderedDeck?.[0]?.content).toBeTruthy()
    expect(getRenderedButton(runtime, 0)).toMatchObject({
      background: '#10161f',
      keyIndex: 0,
      label: 'Clock',
    })
        ...
        buttons: [
          {
            ...
            definition: {
              ...
              render: ({ button }: { button: { position: number } }) =>
                createTextSurface(button.position, currentLabel),
              type: 'display-text',
            },
            label: 'Clock',
            position: 1,
        ...
        buttons: [
          {
            ...
            definition: {
              ...
              render: ({ button }: { button: { position: number } }) =>
                createTextSurface(button.position, 'Broken Tap'),
              type: 'broken-tap',
            },
            label: 'Broken Tap',
            position: 0,
        ...
        buttons: [
          {
            ...
            definition: {
              ...
              render: ({
                button,
                hostContext: receivedHostContext,
              }: {
                button: { position: number }
  ...
  it('resolves button background precedence before runtime render output', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(renderedButtons?.[1]).toMatchObject({
        background: '#445566',
        keyIndex: 1,
        label: 'Button',
      })
      ...
      expect(getRenderedButton(runtime, 1)).toMatchObject({
        background: '#445566',
        keyIndex: 1,
        label: 'Button',
      })
            ...
            definition: {
              ...
              onTap: async ({
                ...
                await methods.runCommand(
                  config.select_command.replaceAll('{{emoji}}', config.emoji),
                )
              },
              render: ({
                button,
        ...
        buttons: [
          {
            ...
            definition: {
              ...
              refresh: async ({
                ...
                await methods.runCommand(
                  "printf '%s|%s' '{{host.os.type}}' '{{host.session.state}}'",
                )
              },
              render: ({ button }: { button: { position: number } }) =>
                createTextSurface(button.position, 'Status'),
  ...
  it('keeps the committed Phase 11 host-context fixture live across render and action execution', async () => {
    ...
    await vi.waitFor(() => {
      expect(executeAction).toHaveBeenCalledWith(
        "printf '%s' '😀 @ {{host.os.type}} @ {{host.session.state}}'",
      )
    })
  })
  ...
  it('navigates generated emoji decks and runs the favorites selection command', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(getRenderedButtonHtml(getRenderedButton(runtime, 0))).toContain(
        'GRIN',
      )
    })
  ...
  it('prefers button interval_ms over definition defaults for polling', async () => {
    const createScheduler = vi.fn(
      (_intervalMs: number): PollingScheduler => ({
        intervalMs: 0,
        jitterMs: 0,
        scheduleDelay: () => 0,
        start: vi.fn(),
  ...
  it('keeps poll and render cadence loops independent when both are configured', async () => {
    ...
    const createScheduler = vi.fn(
      (intervalMs: number): PollingScheduler => ({
        intervalMs,
        jitterMs: 0,
        scheduleDelay: () => 0,
        start: (tasks) => {
    ...
    const pollTask = [...scheduledTasks.entries()].find(([id]) =>
      id.endsWith('-poll'),
    )?.[1]
    const renderTask = [...scheduledTasks.entries()].find(([id]) =>
      id.endsWith('-render'),
    )?.[1]
        ...
        buttons: [
          {
            ...
            definition: createNavigationDefinition(
              'Go to Apps',
              'apps',
              'nav-main-apps',
            ),
        ...
        buttons: [
          {
            ...
            definition: {
              ...
              onTap: async ({
                ...
                methods: { navigateToDeck: (deckId: string) => Promise<void> }
              }) => {
                await methods.navigateToDeck('settings')
              },
              render: ({ button }: { button: { position: number } }) =>
                createTextSurface(button.position, 'Apps'),
  ...
  it('uses the implicit centered five-button time fallback when no locked deck is configured', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(getRenderedButtonHtml(getRenderedButton(runtime, 5))).toContain(
        '0',
      )
      expect(getRenderedButtonHtml(getRenderedButton(runtime, 6))).toContain(
        '9',
      )
      expect(getRenderedButtonHtml(getRenderedButton(runtime, 7))).toContain(
        ':',
      )
      expect(getRenderedButtonHtml(getRenderedButton(runtime, 8))).toContain(
        '0',
      )
      expect(getRenderedButtonHtml(getRenderedButton(runtime, 9))).toContain(
        '8',
      )
    })
        ...
        buttons: [
          {
            ...
            definition: {
              ...
              render: ({ button }: { button: { position: number } }) =>
                createTextSurface(button.position, 'Settings'),
              type: 'nav-settings',
            },
            label: 'Settings',
            position: 0,
          ...
          buttons: [
            {
              ...
              definition: {
                ...
                onTap: async ({
                  ...
                  await methods.navigateToDeck('locked-tools')
                },
                render: ({ button }: { button: { position: number } }) =>
                  createTextSurface(button.position, 'Locked Tools'),
  ...
  it('keeps the committed Phase 24 mounted-button fixture live across loader and runtime execution', async () => {
      ...
      addons: [
        {
          ...
          path: join(FIXTURES_DIRECTORY, 'phase-24/local-mounted-addon'),
          source: 'local',
        },
      ],
    ...
    await vi.waitFor(() => {
      ...
      expect(getRenderedButtonHtml(renderedButton)).toContain(
        'Shared:button=0:addon=0',
      )
      expect(getRenderedButtonHtml(getRenderedButton(runtime, 1))).toContain(
        'Observer:button=0:addon=0',
      )
      expect(getRenderedButtonHtml(getRenderedButton(runtime, 3))).toContain(
        'Local Main:mount=',
      )
      expect(getRenderedButtonHtml(getRenderedButton(runtime, 3))).toContain(
        ':count=0',
      )
      expect(getRenderedButtonHtml(getRenderedButton(runtime, 4))).toContain(
        'Press Probe:frame=idle:pressed=up:count=0',
      )
    })
    ...
    await vi.waitFor(() => {
      expect(getRenderedButtonHtml(getRenderedButton(runtime, 0))).toContain(
        'Shared:button=1:addon=1',
      )
      expect(getRenderedButtonHtml(getRenderedButton(runtime, 1))).toContain(
        'Observer:button=0:addon=1',
      )
    })
    ...
    await vi.waitFor(() => {
      expect(runtime.getActiveDeck().id).toBe('apps')
      expect(getRenderedButtonHtml(getRenderedButton(runtime, 0))).toContain(
        'Apps Observer:button=0:addon=2',
      )
      expect(getRenderedButtonHtml(getRenderedButton(runtime, 1))).toContain(
        'Back Main:button=0:addon=2',
      )
    })
    ...
    await vi.waitFor(() => {
      expect(runtime.getActiveDeck().id).toBe('main')
      expect(getRenderedButtonHtml(getRenderedButton(runtime, 0))).toContain(
        'Shared:button=1:addon=3',
      )
      expect(getRenderedButtonHtml(getRenderedButton(runtime, 1))).toContain(
        'Observer:button=0:addon=3',
      )
      expect(getRenderedButtonHtml(getRenderedButton(runtime, 2))).toContain(
        'Go Apps:button=1:addon=3',
      )
    })
  })
  ...
  it('keeps committed Phase 24 addon-store state across deck changes but resets on rebuilt runtime', async () => {
    ...
    await vi.waitFor(() => {
      expect(
        getRenderedButtonHtml(getRenderedButton(firstRun.runtime, 0)),
      ).toContain('Shared:button=0:addon=0')
    })
    ...
    await vi.waitFor(() => {
      ...
      expect(
        getRenderedButtonHtml(getRenderedButton(firstRun.runtime, 0)),
      ).toContain('Apps Observer:button=0:addon=2')
      expect(
        getRenderedButtonHtml(getRenderedButton(firstRun.runtime, 1)),
      ).toContain('Back Main:button=0:addon=2')
    })
    ...
    await vi.waitFor(() => {
      ...
      expect(
        getRenderedButtonHtml(getRenderedButton(rebuiltRun.runtime, 0)),
      ).toContain('Shared:button=0:addon=0')
      expect(
        getRenderedButtonHtml(getRenderedButton(rebuiltRun.runtime, 1)),
      ).toContain('Observer:button=0:addon=0')
      expect(
        getRenderedButtonHtml(getRenderedButton(rebuiltRun.runtime, 2)),
      ).toContain('Go Apps:button=0:addon=0')
      expect(
        getRenderedButtonHtml(getRenderedButton(rebuiltRun.runtime, 3)),
      ).toContain(':count=0')
    })
  })
  ...
  it('proves committed Phase 24 mounted local state persists while active and resets after deck unmount', async () => {
    ...
    await vi.waitFor(() => {
      const parts = getFixtureMountedLocalParts(
        getRenderedButtonHtml(getRenderedButton(runtime, 3)),
        'Local Main',
      )
    ...
    await vi.waitFor(() => {
      expect(runtime.getActiveDeck().id).toBe('apps')
      const parts = getFixtureMountedLocalParts(
        getRenderedButtonHtml(getRenderedButton(runtime, 2)),
        'Local Apps',
      )
      expect(parts.count).toBe(0)
  ...
  it('proves committed Phase 24 fixture carries runtime-driven transient props through the mounted deck path', async () => {
    ...
    await vi.waitFor(() => {
      expect(getRenderedButton(runtime, 4)).toMatchObject({
        frame_state: 'idle',
        keyIndex: 4,
        label: 'Press Probe',
      })
    ...
    await vi.waitFor(() => {
      ...
      expect(getRenderedButtonHtml(getRenderedButton(runtime, 4))).toContain(
        'Press Probe:frame=hold:pressed=down:count=0',
      )
    })
    ...
    await vi.waitFor(() => {
      ...
      expect(getRenderedButtonHtml(getRenderedButton(runtime, 4))).toContain(
        'Press Probe:frame=idle:pressed=up:count=1',
      )
    })
  }, 10_000)
            ...
            definition: {
              ...
              render: ({ button }: { button: { position: number } }) =>
                createElement(
                  ButtonSurface,
                  { full: true },
                  createTextSurface(button.position, 'Clock'),
                ),
  ...
  it('executes mounted button definitions through runtime-owned press, release, and tap semantics', async () => {
        ...
        buttons: [
          {
            ...
            definition: defineMountedButton({
              ...
              render: (props) =>
                createTextSurface(
                  props.button.position,
                  `${props.config.label}:${props.frameState}:${props.pressed ? 'down' : 'up'}`,
                ),
              type: 'mounted-text',
            }),
            label: 'Mounted',
    ...
    await vi.waitFor(() => {
      expect(onPress).toHaveBeenCalledWith(
        expect.objectContaining({ frameState: 'hold', pressed: true }),
      )
      expect(getRenderedButtonHtml(getRenderedButton(runtime, 0))).toContain(
        'Mounted:hold:down',
      )
    ...
    await vi.waitFor(() => {
      expect(onRelease).toHaveBeenCalledWith(
        expect.objectContaining({ frameState: 'idle', pressed: false }),
      )
      expect(onTap).toHaveBeenCalledWith(
        expect.objectContaining({ frameState: 'tap', pressed: false }),
      )
      expect(getRenderedButtonHtml(getRenderedButton(runtime, 0))).toContain(
        'Mounted:idle:up',
      )
  ...
  it('exposes runtime-owned mounted store scopes for button-local and addon-wide coordination', async () => {
    let emitEvent: ((event: StreamDeckKeyEvent) => void) | undefined
    const sharedDefinition = setAddonButtonOwnerName(
      defineMountedButton({
        configSchema: {
          parse: (value: unknown) => value,
          safeParse: (value: unknown) => ({
        ...
        onTap: ({ store }) => {
          ...
          store.addon.update((snapshot) => ({
            total:
              ((snapshot as { total?: number } | undefined)?.total ?? 0) + 1,
          }))
        },
    ...
    const observerDefinition = setAddonButtonOwnerName(
      defineMountedButton({
        configSchema: {
          parse: (value: unknown) => value,
          safeParse: (value: unknown) => ({
  ...
  it('preserves mounted local component state while a deck stays active but resets it after deck navigation unmounts the host', async () => {
    ...
    function MountedCounterView(props: { count: number; label: string }) {
      ...
      return createElement(
        'span',
        null,
        `${props.label}:${mountId}:${props.count}`,
      )
    }

    function createMountedCounterDefinition(
      label: string,
      targetDeckId?: string,
    ) {
      return defineMountedButton({
        ...
        onTap: async ({ methods, store }) => {
          store.button.update((snapshot) => ({
            count:
              ((snapshot as { count?: number } | undefined)?.count ?? 0) + 1,
          }))
        ...
        render: ({ button, config, store }) => {
          const count =
            (store.button.snapshot as { count?: number } | undefined)?.count ??
            0
    ...
    await vi.waitFor(() => {
      const parts = getMountedCounterParts(
        getRenderedButtonHtml(getRenderedButton(runtime, 0)),
        'Main Counter:0',
      )
      expect(parts.count).toBe(0)
      initialMainMountId = parts.mountId
  ...
  it('reconciles toggle-status taps through status_command instead of local inversion', async () => {
    ...
    await vi.waitFor(() => {
      expect(executeAction.mock.calls.map((call) => call[0])).toEqual([
        'read-lamp',
        'toggle-lamp',
        'read-lamp',
      ])
  ...
  it('allows toggle-status taps before the first status read has resolved', async () => {
    ...
    await vi.waitFor(() => {
      expect(executeAction.mock.calls.map((call) => call[0])).toContain(
        'toggle-lamp',
      )
      expect(getRenderedButtonHtml(getRenderedButton(runtime, 0))).toContain(
        'OFF',
  ...
  it('keeps the settled get-set startup render from being overwritten by the stale deck-wide pending write', async () => {
    ...
    expect(renderedDeckButton).toMatchObject({
      background: '#10161f',
      keyIndex: 0,
      label: 'Lamp',
    })
  ...
  it('re-renders pressed frame state on down and returns to idle on up while preserving tap behavior', async () => {
        ...
        buttons: [
          {
            ...
            definition: {
              ...
              render: ({ button }: { button: { position: number } }) =>
                createTextSurface(button.position, 'Press Me'),
    ...
    expect(onRenderDeck.mock.calls.at(-1)?.[0]?.[0]).toMatchObject({
      frame_state: 'idle',
      keyIndex: 0,
      label: 'Press Me',
    })
  ...
  it('treats paginated deck navigation as a noHistory page-to-page flow (49-07)', async () => {
    ...
    const runtime = createDeckRuntime({
      ...
      decks: {
        ...
        'cat-p2': {
          buttons: [
            pageNavButton(0, 2, 5, 'cat-p1', 'cat-p3'),
            goBackButton(1),
          ],
          id: 'cat-p2',
        },
        'cat-p3': {
          buttons: [
            pageNavButton(0, 3, 5, 'cat-p2', 'cat-p4'),
            goBackButton(1),
          ],
          id: 'cat-p3',
        },
        'cat-p4': {
          buttons: [
            pageNavButton(0, 4, 5, 'cat-p3', 'cat-p5'),
            goBackButton(1),
          ],
          id: 'cat-p4',
        },
  ...
  it('renders the implicit settings deck with brightness up/down and logo+version (54-02)', async () => {
    ...
    const logoVersionHtml = getRenderedButtonHtml(getRenderedButton(runtime, 3))
    ...
    expect(brightnessDownHtml).toContain(
      'sireno-settings-button="brightness-down"',
    )
    expect(currentBrightnessHtml).toContain('50%')
    expect(currentBrightnessHtml).toContain(
      'sireno-settings-button="current-brightness"',
    )

    emitEvent?.({ keyIndex: 0, type: 'down' })
    emitEvent?.({ keyIndex: 0, type: 'up' })
    await vi.waitFor(() => {
      const updatedHtml = getRenderedButtonHtml(getRenderedButton(runtime, 2))
