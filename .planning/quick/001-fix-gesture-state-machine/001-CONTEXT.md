...
`packages/cli/emulator/src/gesture.ts`.
</domain>
...
- `nextGesture` returns `tap` from the cleanup at end of `await-second` state
  _immediately_ after `[down, up]`. There's no delay for a potential second
  tap → dbl-tap upgrade.
...
- User verbatim: _"after the frist click everywinth is detected as double tap,
  hold is not beeingh triggered"_.
