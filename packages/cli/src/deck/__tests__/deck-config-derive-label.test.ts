...
describe("buildDeckConfigMessage — deriveLabel for core:action", () => {
  ...
  it("uses config.command when present (legacy behavior)", () => {
    ...
    const msg = buildDeckConfigMessage(deck, new Map(), {}, undefined, 15, false, () => undefined)
  ...
  it("truncates long commands but uses full label", () => {
    ...
    const msgCmd = buildDeckConfigMessage(deckCmd, new Map(), {}, undefined, 15, false, () => undefined)
    ...
    const msgLabel = buildDeckConfigMessage(deckLabel, new Map(), {}, undefined, 15, false, () => undefined)
