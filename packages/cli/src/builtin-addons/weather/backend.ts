import type { AddonBackendContext, AddonGlobalBackend } from '@/addon/api'
...
let ctxRef: AddonBackendContext | undefined
...
export const globalBackend: AddonGlobalBackend = {
  ...
  pollers: [
    {
      ...
      poll: async (ctx: AddonBackendContext): Promise<WeatherStateSnapshot> => {
  ...
  onLoad: async (ctx: AddonBackendContext) => {
    ctxRef = ctx
  },
  onUnload: () => {
    ctxRef = undefined
