...
describe("parseCombo", () => {
  ...
  it("parses multiple modifiers", () => {
    expect(parseCombo("alt+shift+F4")).toEqual({ mods: ["alt", "shift"], key: "F4" });
  });
  ...
  it("tolerates extra whitespace", () => {
    expect(parseCombo("  ctrl  +  shift  +  a  ")).toEqual({ mods: ["ctrl", "shift"], key: "a" });
  });
...
describe("knownKeys", () => {
  it("contains at least the common control keys", () => {
    for (const key of ["Return", "Tab", "Escape", "BackSpace", "Up", "Down", "Left", "Right"]) {
