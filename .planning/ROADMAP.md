...
| 16  | migrate-weather-addon | ✅ done       | Migrate the weather addon from legacy to new typed addon paradigm: export WeatherSnapshot/WeatherLocation types from config.ts, fix broken import paths in fetch.ts, add wmoCode to poller return, type frontend component with AddonFrontendButton<ConfigSchema>, fix invalid tone values, fix index.d.ts manifest type reference. |
