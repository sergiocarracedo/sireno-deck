...
- Manual device UAT passed for the browser-rendered action, live DOM, and sampled-media fixtures.
- No known implementation or procedural blocker remains for Phase 18 verification.

## Must-have coverage

1. Browser-backed DOM rendering is the shipped path for the bundled Phase 18 buttons.
   Evidence:
   - `packages/cli/src/cli/commands/start.ts` sends all-DOM decks through `renderDomDeckSurface()` and `createBrowserRenderer()`.
   - Bundled `action`, `change-deck`, `toggle`, `date-time`, `analog-clock`, `calendar-sheet`, and `media-demo` buttons all return `createDomButtonRender(...)`.
   - `packages/cli/src/builtin-addons/core-buttons/index.test.ts` asserts DOM render output for shipped core buttons.
...
3. Live DOM invalidation stays coherent at the deck level.
   Evidence:
   - `packages/cli/src/deck/runtime.ts` rerenders the full active DOM deck when a DOM-authored button invalidates or refreshes.
   - `packages/cli/src/render/browser-renderer.ts` keeps only the latest pending browser capture.
   - `.planning/phases/18-react-dom-based-renderer-with-htmlcss/18-UAT.md` Fixture 2 covers real-device validation for live DOM buttons.

4. Rich media is supported as bounded sampled capture, not continuous playback.
   Evidence:
   - `packages/cli/src/addon/api.ts` exposes `sample_interval_ms` on DOM-authored button renders.
...
- Mixed DOM and legacy SVG decks still use the compatibility render path until later migration work removes it completely.

## Residual compatibility seam

- The old SVG/text-image helper path still exists as a compatibility seam for non-DOM buttons.
