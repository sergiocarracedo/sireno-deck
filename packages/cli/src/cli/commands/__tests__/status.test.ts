...
const TEST_DIR = join(tmpdir(), `sireno-deck-status-test-${process.pid}`)
...
describe("status", () => {
  ...
  it("reports stale pid file with no other artifacts", async () => {
    writeFileSync(join(TEST_DIR, "sireno-deck.pid"), "99999999\n", "utf8")
    const logger = silentLogger()
  ...
  it("reports running daemon with token preview + children", async () => {
    ...
    writeFileSync(
      join(TEST_DIR, "sireno-deck.token"),
