...
const buildPageItem = <T>(id: string, value: T): PaginatedItem<T> => ({ id, value });
...
export const paginate = <T>(
  ...
  for (let pageIndex = 0; pageIndex < totalPages; pageIndex++) {
    ...
    if (!isLast && pageItems.length >= pageSize) {
      const markerIndex = pageItems.length >= pageSize ? pageSize - 1 : pageItems.length;
      pageItems.splice(markerIndex, 0, NEXT_PAGE_MARKER as unknown as PaginatedItem<T>);
    }
