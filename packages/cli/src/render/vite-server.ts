  ...
  return new Promise((resolve, reject) => {
    ...
    const start = (): void => {
      ...
      current.stdout?.on("data", (chunk: Buffer) => {
        ...
        if (port === null) {
          const match =
            text.match(readyMatcher) ?? buffer.join("").match(readyMatcher)
      ...
      current.on("exit", (code) => {
        ...
        if (port === null) {
          reject(
            new Error(
              `vite spawn: process exited with code ${code} before READY`,
            ),
          )
          return
        }
        ...
        const backoff =
          restartBackoffMs[
            Math.min(restartCount, restartBackoffMs.length - 1)
          ] ?? 1000
        restartCount += 1
