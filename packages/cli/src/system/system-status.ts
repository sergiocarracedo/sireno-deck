...
function formatMetricValue(
  ...
  switch (formatter) {
    case "bytes":
      return numbro(metric.value).format({ average: true, base: "decimal", mantissa: 1, output: "byte", trimMantissa: true })
    case "count":
      return numbro(metric.value).format({ average: true, mantissa: metric.value >= 100 ? 0 : 1, trimMantissa: true })
    case "frequency-ghz":
...
function getDefaultFormatter(metricId: SystemMetricId): SystemStatusFormatter | undefined {
  switch (metricId) {
    case "cpu_usage":
    case "memory_usage":
    case "swap_usage":
...
function getDefaultUnits(metric: CanonicalSystemMetricSnapshot): string | undefined {
  switch (metric.id) {
    case "cpu_usage":
    case "memory_usage":
    case "swap_usage":
...
export function toSystemStatusDisplayMetrics(
  metrics: readonly CanonicalSystemMetricSnapshot[],
  overridesByMetricId: Partial<Record<SystemMetricId, SystemStatusMetricOverride>> = {},
): SystemStatusDisplayMetric[] {
  return metrics.map((metric) => toSystemStatusDisplayMetric(metric, overridesByMetricId[metric.id]))
}
