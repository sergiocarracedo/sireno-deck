...
const expandDeckButtons = (
  ...
  return { ...deck, buttons: expandArray(buttons, configDir) as RawButtonEntry[] };
};

export const expandButtonReferences = (raw: unknown, configDir: string): unknown => {
  if (raw === null || typeof raw !== "object") return raw;
  if (Array.isArray(raw)) return raw;
  const obj = raw as Record<string, unknown>;
  const decks = obj["decks"];
  if (decks !== undefined && decks !== null && typeof decks === "object" && !Array.isArray(decks)) {
    ...
    for (const [id, deck] of Object.entries(decks as Record<string, unknown>)) {
      if (deck !== null && typeof deck === "object" && !Array.isArray(deck)) {
        expandedDecks[id] = expandDeckButtons(deck as Record<string, unknown>, configDir);
      } else {
