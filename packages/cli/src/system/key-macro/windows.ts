...
import { ProviderError, type KeyMacroProvider, withTimeout } from "@/system/provider";
...
const escapeForPowerShellSingleQuote = (s: string): string => s.replace(/'/g, "''");
...
export const createWindowsKeyMacroProvider = async (
  ...
  deps.logger.info("Windows key-macro provider initialised (PowerShell SendKeys)");
    ...
    async sendKey(input: string) {
      ...
      try {
        const result = await withTimeout(
          deps.executor.run("powershell", ["-NoProfile", "-Command", script], { timeoutMs }),
          timeoutMs + 500,
        );
