...
Automated checks pass across theme package loading, shared asset resolution, the centered implicit locked `HH:MM` fallback, URL normalization, and the real browser-renderer capture seam that previously broke local image assets. The remaining work is still human verification on the actual browser/device path: rerun Fixture 2 to confirm the shipped path now shows the addon/config images instead of the square plus broken-image indicator.
...
- Human rerun outcome after `20-05`: **pending**. Re-run `packages/cli/fixtures/phase-20/config.asset-pipeline.yml` on the shipped browser/device path.
