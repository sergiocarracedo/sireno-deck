...
const TEST_DIR = join(tmpdir(), `sireno-deck-2-http-test-${process.pid}`);
...
beforeEach(() => {
  ...
  writeIndexHtml('<!doctype html><html><head><script type="module" src="/assets/main.js"></script></head><body>x</body></html>');
...
const fetchText = async (url: string): Promise<{ status: number; headers: Record<string, string>; body: string }> => {
...
describe("startHttpServer", () => {
  ...
  it("serves /assets/* with correct content-type", async () => {
    ...
    try {
      const res = await fetchText(`http://127.0.0.1:${server.port}/assets/main.js`);
      expect(res.status).toBe(200);
  ...
  it("returns 403 for path traversal attempts", async () => {
    ...
    try {
      const res = await fetchText(`http://127.0.0.1:${server.port}/assets/../etc/passwd`);
