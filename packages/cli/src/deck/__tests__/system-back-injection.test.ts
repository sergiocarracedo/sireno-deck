...
const state = (overrides: Partial<Parameters<typeof computeSystemButtonForSlotN1>[1]> = {}) => ({
  navStackDepth: 1,
...
describe("computeSystemButtonForSlotN1", () => {
  ...
  it("overlay deck returns overlay-toggle", () => {
    expect(computeSystemButtonForSlotN1(deck({ isOverlay: true }), state())).toBe(
      "core:overlay-toggle",
    );
  });

  it("regular deck with navStackDepth=1 returns null", () => {
    expect(computeSystemButtonForSlotN1(deck(), state({ navStackDepth: 1 }))).toBeNull();
  });

  it("regular deck with navStackDepth=2 returns back", () => {
    expect(computeSystemButtonForSlotN1(deck(), state({ navStackDepth: 2 }))).toBe("core:back");
  });
  ...
  it("overlay deck with navStackDepth=3 returns overlay-toggle", () => {
    expect(
      computeSystemButtonForSlotN1(deck({ isOverlay: true }), state({ navStackDepth: 3 })),
