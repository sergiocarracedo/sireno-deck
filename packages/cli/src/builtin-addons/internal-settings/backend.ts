...
import { createBrightnessProvider, type BrightnessProvider } from "@/system/brightness";
...
const noOpLogger = { info: () => undefined, warn: () => undefined } as unknown as pino.Logger;
...
export const globalService: AddonGlobalService = {
  methods: {
    brightnessUp: async (...args: readonly unknown[]) => {
      ...
      await brightnessProvider.setBrightness(Math.min(100, current.value + step));
    },
