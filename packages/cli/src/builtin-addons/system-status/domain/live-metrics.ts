...
const collectMetric = (id: SystemStatusMetricId): CanonicalSystemMetricSnapshot => {
  switch (id) {
    case "cpu_usage": {
      const cpus = (globalThis as { os?: { cpus?: () => unknown[] } }).os?.cpus?.();
    ...
    case "memory_usage": {
      const os = (globalThis as { os?: { totalmem?: () => number; freemem?: () => number } }).os;
    ...
    case "load_average_1m": {
      ...
      return {
        ...
        ...(typeof load === "number" ? { value: load, formattedValue: load.toFixed(2) } : {}),
