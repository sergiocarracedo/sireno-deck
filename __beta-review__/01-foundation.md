...
### [x] [01-foundation #8] [P1] `pnpm typecheck` is broadly red
...
### [x] [01-foundation #9] [P1] Two conflicting formatter configs
...
### [x] [01-foundation #10] [P1] Token storage falls back to `/tmp` when `XDG_RUNTIME_DIR` is absent
...
### [x] [01-foundation #11] [P1] PID identity check uses `process.kill(pid, 0)` only
...
### [x] [01-foundation #12] [P1] Runtime metadata writes are not atomic
...
### [x] [01-foundation #13] [P1] Daemon signal cleanup is split across implementations
...
### [x] [01-foundation #14] [P1] Two concurrent `start` invocations can race
...
### [x] [01-foundation #15] [P1] `killPortListeners` regex is fragile
...
### [x] [01-foundation #16] [P1] `spawn-daemon.ts` log line-by-line `appendFileSync`
...
### [x] [01-foundation #17] [P1] `spawn-daemon.ts:resolveInterpreter` tsx fallback drops tsconfig
...
### [x] [01-foundation #18] [P1] `isOrphan` returns `true` on non-Linux
