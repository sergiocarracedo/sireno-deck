import type { WeatherButtonConfig } from "../buttons/weather/config"
import { WeatherSnapshot } from "./types"

import { describeWeatherCode } from "./codes"
import type { GeocodedLocation } from "./geocode"
import { DailyForecastEntry, HourlyForecastEntry } from "./types"
    ...
    for (let i = 0; i < Math.min(24, json.hourly.time.length); i++) {
      ...
      if (!isNaN(hour)) {
        hourly.push({
          ...
          wmoCode: json.hourly.weather_code?.[i] ?? 0,
  ...
  if (json.daily?.time) {
    for (let i = 0; i < json.daily.time.length; i++) {
      daily.push({
        ...
        wmoCode: json.daily.weather_code?.[i] ?? 0,
