...
export const createDarwinBrightnessProvider = (
  ...
  const script = 'tell application "System Events" to get brightness of (every item of displays)';
  ...
  const getCurrent = async (): Promise<BrightnessReading> => {
    ...
    if (r.exitCode !== 0) {
      logger.warn({ exitCode: r.exitCode, stderr: r.stderr }, "brightness: osascript get failed");
      return createNullBrightnessProvider(logger).getCurrent();
    }
    const parsed = parseOsascript(r.stdout);
    if (parsed === null) {
      logger.warn({ stdout: r.stdout.slice(0, 200) }, "brightness: osascript output not parseable");
      return createNullBrightnessProvider(logger).getCurrent();
    }
    return parsed;
  };
  ...
  const setBrightness = async (value: number): Promise<void> => {
    ...
    if (r.exitCode !== 0) {
      logger.warn({ exitCode: r.exitCode, stderr: r.stderr }, "brightness: osascript set failed");
    }
  };
