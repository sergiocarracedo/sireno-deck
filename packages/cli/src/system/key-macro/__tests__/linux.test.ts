...
describe("createLinuxKeyMacroProvider", () => {
  ...
  it("sendKey literal text uses `type --` syntax with xdotool", async () => {
    ...
    expect(
      calls.find((c) => c.tool === "xdotool" && c.args[0] === "type"),
    ).toEqual({
      tool: "xdotool",
      args: ["type", "--", "hello"],
    })
  ...
  it("throws ProviderError with code EXEC_FAILED when tool exits non-zero", async () => {
    ...
    await expect(provider.sendKey("ctrl+Insert")).rejects.toBeInstanceOf(
      ProviderError,
    )
    await expect(provider.sendKey("ctrl+Insert")).rejects.toMatchObject({
      code: "EXEC_FAILED",
    })
    await provider.stop()
  })
