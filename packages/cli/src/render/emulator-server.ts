...
export const startEmulatorServer = async (
  ...
  const child = spawn(
    "node",
    ["--import", "tsx/esm", viteBin, "--port", String(port), cliEntry],
    {
    ...
    const onData = (chunk: Buffer): void => {
      ...
      for (const line of text.split("\n")) {
        if (line.startsWith(READY_PREFIX)) {
          const actualPort = Number.parseInt(
            line.slice(READY_PREFIX.length).trim(),
            10,
          )
...
export const emulatorServerEntryExists = (
  frontendEmulatorDir: string,
): boolean => {
