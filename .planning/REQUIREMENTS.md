...
- **Phase 23 follow-on authoring/startup work:** post-roadmap addon/runtime polish on top of the shipped browser path — manifest-driven local raw `.ts/.tsx/.jsx` addon loading through root exports only, bounded local source-graph loading, a branded hardware startup placeholder that clears on first render or honest startup failure, and a corrected shipped sample config plus regression coverage that keep fixture button `type` values aligned with real registered addon button ids.
