...
describe("text-image", () => {
  ...
  it("renders get-set toggle mode with observably different chrome from internal mode on the same toggle family", async () => {
    const internalToneBuffer = await renderTextImage({ text: "Lamp", subtitle: "ON", toggleMode: "internal", variant: "toggle", theme: createTheme() })
    const getSetToneBuffer = await renderTextImage({ text: "Lamp", subtitle: "ON", toggleMode: "get-set", variant: "toggle", theme: createTheme() })

    expect(internalToneBuffer.equals(getSetToneBuffer)).toBe(false)
