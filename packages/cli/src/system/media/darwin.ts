...
const runScript = async (deps: DarwinMediaDeps, script: string): Promise<void> => {
  try {
    ...
    if (result.exitCode !== 0) {
      throw new ProviderError("EXEC_FAILED", `osascript: ${result.stderr.trim() || "exit " + result.exitCode}`);
    }
  } catch (err) {
...
export const createDarwinMediaProvider = async (
  ...
  const pollOnce = async (): Promise<MediaMetadata | null> => {
    try {
      const result = await deps.executor.run("osascript", ["-e", SCRIPT_METADATA], { timeoutMs: 2_000 });
  ...
  let interval: ReturnType<typeof setInterval> | null = setInterval(() => {
    ...
    void pollOnce().then((m) => {
      if (stopped) return;
      const same = lastMeta !== null && m !== null && lastMeta.title === m.title && lastMeta.artist === m.artist;
      if (!same) {
        lastMeta = m;
