...
### [x] [04-security #2] [P0] S6 — HTTP server exposes raw config, paths, addon metadata
...
### [x] [04-security #3] [P0] S12 — `validateAndLoadConfig` reads YAML from disk via `getOriginalCwd()`
...
### [x] [04-security #4] [P0] S5 — PID file identity is not verified
...
### [x] [04-security #5] [P0] S8 — Service log opened by parent + child without locking
...
### [x] [04-security #6] [P1] S14 — Token storage falls back to `/tmp` on Linux
