...
describe("resolveIconRef", () => {
  ...
  it("resolves icon://<id> to CLI builtin", () => {
    expect(resolveIconRef("icon://play", ctx)).toEqual({
      kind: "cli-builtin",
      id: "play",
    })
  })
  ...
  it("rejects unknown CLI builtin ids", () => {
    expect(() => resolveIconRef("icon://nope", ctx)).toThrow(
      /Unknown CLI-builtin icon/,
    )
  })
  ...
  it("rejects malformed builtin:// ref", () => {
    expect(() => resolveIconRef("builtin://noSlash", ctx)).toThrow(
      /Invalid builtin/,
    )
  })
  ...
  it("rejects unrecognized refs", () => {
    expect(() => resolveIconRef("not-a-uri-just-a-word", ctx)).toThrow(
      /Unrecognized icon ref/,
    )
  })
})
...
describe("iconSourceToString", () => {
  it("round-trips each kind", () => {
    expect(iconSourceToString({ kind: "cli-builtin", id: "play" })).toBe(
      "icon://play",
    )
    expect(
      iconSourceToString({
    ...
    ).toBe("addon://third/x.svg")
    expect(iconSourceToString({ kind: "path", absolutePath: "/a/b.png" })).toBe(
      "/a/b.png",
    )
  })
})
...
describe("expandButtonReferences", () => {
  it("returns the input unchanged when there are no decks", () => {
    expect(expandButtonReferences({ foo: "bar" }, "/tmp")).toEqual({
      foo: "bar",
    })
  })
  ...
  it("expands @file.yml in buttons", () => {
    ...
    const result = expandButtonReferences(raw, dir) as {
      decks: { main: { buttons: unknown[] } }
    }
    expect(result.decks.main.buttons).toHaveLength(3)
    expect(result.decks.main.buttons[0]).toEqual({
      type: "core:play",
      position: 0,
    })
    expect(result.decks.main.buttons[2]).toEqual({
      type: "core:settings",
      position: 2,
    })
  })
...
describe("findConfigPath", () => {
  it("prefers --config explicit path", () => {
    ...
    expect(findConfigPath({ explicitPath: explicit, homeDir: "/tmp" })).toBe(
      explicit,
    )
  })
  ...
  it("falls back to cwd/config.yml", () => {
    const dir = makeTempDir()
    writeFileSync(
      join(dir, DEFAULT_CONFIG_FILENAME),
      "decks:\n  main:\n    buttons: []\n",
    )
    expect(findConfigPath({ cwd: dir, homeDir: "/tmp" })).toBe(
      join(dir, DEFAULT_CONFIG_FILENAME),
    )
  })
  ...
  it("returns null when nothing is found", () => {
    const dir = makeTempDir()
    expect(
      findConfigPath({ cwd: dir, homeDir: dir, xdgConfigHome: dir }),
    ).toBeNull()
  })

  it("does not walk up parent directories", () => {
    ...
    expect(findConfigPath({ cwd: child, homeDir: "/tmp" })).toBeNull()
  })
