...
**Objective:** real npm addon loading via `npm install --prefix ~/.cache/sireno-deck/`.

**Status: ✓ passed** (with one known follow-up)

| Must-have                                                                                                              | Verified                                                                                            |
| ---------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| Cross-platform cache dir (`~/.cache/sireno-deck/` on Linux, `~/Library/Caches/` on macOS, `%LOCALAPPDATA%` on Windows) | ✓ — `cache-paths.test.ts` covers XDG path + lazy mkdir                                              |
