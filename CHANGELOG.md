...
- **`type://` replaces `macro://`; `paste://` removed.** Single keystroke-injection scheme across all OSes. Linux provider uses `ydotool` (uinput — works on GNOME Wayland where `wtype`'s virtual-keyboard protocol is unsupported); `wtype` is a wlroots-only fallback when ydotool is absent. Combos emit `ydotool key <scancode>:<value>` (since `key ctrl+v` symbolic form is not understood by ydotool). Non-ASCII literal text (emoji, accented letters, CJK) routes through `wl-copy` + ydotool ctrl+v scancode injection because `ydotool type` does not handle non-BMP characters. Windows provider uses Win32 `SendInput` via inline C# compiled at provider init (`KEYEVENTF_UNICODE` for text, VK codes for combos), with `Marshal.GetLastWin32Error()` mapped into `ProviderError`; macOS still uses `osascript keystroke`. A linux clipboard provider is restored internally (not on `Methods`) for the non-ASCII bridge. New `Methods.typeText(text)` ergonomic for addons that want to type literal text without building a `KeyMacroAction`.
