...
describe("createDarwinMediaProvider", () => {
  ...
  it("play() invokes osascript 'tell application Spotify to play'", async () => {
    ...
    const provider = await createDarwinMediaProvider({ executor, logger: silentLogger() });
    await provider.play();
  ...
  it("getCurrent() parses Spotify record output", async () => {
    ...
    expect(meta).toEqual({ title: "Time", artist: "Pink Floyd", album: "Dark Side of the Moon", artUrl: null });
    await provider.stop();
  });

  it("getCurrent() returns null on empty output", async () => {
    const executor = makeExecutor(() => ({ exitCode: 1, stdout: "", stderr: "no player" }));
    const provider = await createDarwinMediaProvider({ executor, logger: silentLogger() });
    const meta = await provider.getCurrent();
  ...
  it("onChange handler fires when track changes", async () => {
    ...
    const provider = await createDarwinMediaProvider({ executor, logger: silentLogger(), pollIntervalMs: 100 });
    const handler = vi.fn();
