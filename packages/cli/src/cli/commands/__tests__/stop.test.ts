...
const TEST_DIR = join(tmpdir(), `sireno-deck-2-stop-test-${process.pid}`);
...
const writeDaemonFiles = (pid: number, childPids: number[]): void => {
  ...
  writeFileSync(join(TEST_DIR, "sireno-deck-2.token"), "test-token\n", "utf8");
  writeFileSync(
...
describe("stop", () => {
  it("returns silently when no daemon is running", async () => {
    ...
    expect(existsSync(join(TEST_DIR, "sireno-deck-2.pid"))).toBe(false);
  });

  it("removes stale pid file and reports no running daemon", async () => {
    writeFileSync(join(TEST_DIR, "sireno-deck-2.pid"), "999999\n", "utf8");
    await stop({ logger: silentLogger() });
    expect(existsSync(join(TEST_DIR, "sireno-deck-2.pid"))).toBe(false);
  });
  ...
  it("kills a real running daemon with its children", async () => {
    ...
    expect(existsSync(join(TEST_DIR, "sireno-deck-2.pid"))).toBe(false);
    expect(existsSync(join(TEST_DIR, "sireno-deck-2.token"))).toBe(false);
    expect(existsSync(join(TEST_DIR, "sireno-deck-2.children.json"))).toBe(false);
  });
