...
```typescript
// manifest.ts
import type { AddonManifestV1 } from "@sireno-deck/cli"
  ...
  decks: [
    {
      ...
      icon: "icon://menu",
      buttons: [
        { position: 0, type: "my-addon:greet" }
      ]
    }
  ]
}
...
import type { AddonFrontendButtonProps } from "@sireno-deck/cli"
...
```typescript
// backend.ts
import type { AddonButtonTypeService } from "@sireno-deck/cli"
...
decks: [
  {
    ...
    name: "Static Deck",
    buttons: [
      { position: 0, type: "my-addon:action" }
    ]
  }
]
```
