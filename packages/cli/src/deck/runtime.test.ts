...
describe("createDeckRuntime", () => {
  ...
  it("preserves config-authored full-surface metadata on bundled action runtime output", async () => {
    const registry = createBundledAddonRegistry()
    const onRenderDeck = vi.fn()
    const runtime = createDeckRuntime({
      addonRegistry: registry,
      deck: {
        ...
        buttons: [{
          config: { label: "Clock" },
          definition: registry.getButton("action")!,
          full_surface: true,
          label: "Clock",
          position: 0,
          type: "action",
  ...
  it("preserves config-authored primitive ids on bundled action runtime output", async () => {
    const registry = createBundledAddonRegistry()
    const onRenderDeck = vi.fn()
    const runtime = createDeckRuntime({
      addonRegistry: registry,
