...
describe("createLinuxProvider", () => {
  it("maps Playing/Paused/Stopped to play/pause/stop", async () => {
    ...
    const provider2 = createLinuxProvider({
      executor: makeExecutor(baseResponses("Paused")),
    })
    expect((await provider2.getStatus()).playStatus).toBe("pause")

    const provider3 = createLinuxProvider({
      executor: makeExecutor(baseResponses("Stopped")),
    })
  ...
  it("returns 'unavailable' for unknown status strings", async () => {
    const provider = createLinuxProvider({
      executor: makeExecutor(baseResponses("WeirdStatus")),
    })
  ...
  it("setVolume calls wpctl set-volume with the absolute percentage", async () => {
    ...
    expect(run).toHaveBeenCalledWith(
      "wpctl",
      ["set-volume", "@DEFAULT_AUDIO_SINK@", "42%"],
      {
        timeoutMs: 5_000,
      },
    )
  ...
  it("volumeDown calls wpctl set-volume with relative %-", async () => {
    ...
    expect(run).toHaveBeenCalledWith(
      "wpctl",
      ["set-volume", "@DEFAULT_AUDIO_SINK@", "10%-"],
      {
        timeoutMs: 5_000,
      },
    )
