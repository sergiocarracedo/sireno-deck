...
const logNull = (logger: pino.Logger | undefined, name: string, reason: string): void => {
  if (logger) {
    logger.warn({ provider: name, reason }, "OS provider unavailable, using null provider");
  }
};
...
export const createNullSessionProvider = (logger?: pino.Logger): SessionProvider => {
  if (logger) {
    logger.warn({ provider: "session" }, "OS session provider unavailable, using null provider");
  }
  return {
    getState() {
      return "unknown";
...
export const createNullKeyMacroProvider = (logger?: pino.Logger): KeyMacroProvider => {
  if (logger) {
    logger.warn(
      { provider: "key-macro" },
