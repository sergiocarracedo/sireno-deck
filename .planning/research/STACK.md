...
- **No DOM in addon rendering** — React components use a custom reconciler host config (`packages/cli/src/render/`), not the browser DOM. The frontend Vite SPA _does_ use the real DOM, but addon authors don't import from `react-dom`.
