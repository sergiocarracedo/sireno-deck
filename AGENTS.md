...
- Plan 76-04 (wave 3): frontend button actions (shell scope)
- Plan 76-05 (wave 4): gesture state port + system surfaces (port SplitActionSurface from `packages/cli/src/ui/surfaces/` to Vite frontend; create SystemReservedBack + ErrorSurface)
- Plan 76-06 (wave 5): overlay lifecycle + hardware sync
- Plan 76-07 (wave 6): hard cut runtime.ts + dom-host/ retirement (DELETES `packages/cli/src/render/dom-host/` and the CLI-side SplitActionSurface; deletes obsolete runtime.test.ts tests for Phase 42/67 system-back-injection code; the 79-failure known issue is resolved by deleting the tests for the deleted production code)

Scope decisions in `.planning/phases/76-arch-migration-runtime-and-theme/76-DISCUSSION-LOG.md` (D-01..D-07). Open gray areas G-01..G-07 resolved in the plans. Plan-checker caught and fixed: wave inversions, file-path errors (real paths: `packages/cli/frontend/src/`, `packages/cli/src/render/`, `packages/cli/src/addon/registry.ts`, `packages/cli/src/render/dom-host/`), single-string `addonManifest.frontend`, existing `{ok: false, error}` method-call shape, `command.exec` reuses existing `packages/cli/src/action/executor.ts` (no new `runner/command-runner.ts`), 76-04 scoped to SHELL only (per-addon frontend authoring is a future phase — only `date-time/frontend.tsx` exists today; other addons are CLI-side TSX).
