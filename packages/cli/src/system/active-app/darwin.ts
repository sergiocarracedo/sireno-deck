...
export const createDarwinActiveAppProvider = async (
  ...
  const snapshot = async (): Promise<ActiveAppSnapshot | null> => {
    try {
      const result = await deps.executor.run("osascript", ["-e", APPLE_SCRIPT_GET_ACTIVE], { timeoutMs: 3_000 });
  ...
  interval = setInterval(() => {
    ...
    void snapshot().then((s) => {
      if (stopped) return;
      const same = last !== null && s !== null && last.name === s.name && last.windowTitle === s.windowTitle && last.processId === s.processId;
      if (!same) {
        last = s;
