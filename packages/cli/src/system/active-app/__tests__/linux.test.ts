...
const dbusStub = (): { bus: LinuxDbusBus; evalMock: ReturnType<typeof vi.fn> } => {
...
describe("createLinuxActiveAppProvider", () => {
  ...
  it("returns parsed snapshot from D-Bus Eval", async () => {
    ...
    expect(snap).toEqual({ name: "Google Chrome", windowTitle: "GitHub", processId: 1234 });
  ...
  it("falls back to /proc when D-Bus throws", async () => {
    ...
    const executor = makeExecutor((cmd, args) => {
      if (cmd === "sh" && args[0] === "-c" && args[1]?.startsWith("xdotool getactivewindow")) {
        return { exitCode: 0, stdout: "0x1234", stderr: "" };
      }
      if (cmd === "sh" && args[0] === "-c" && args[1]?.startsWith("xdotool getwindowpid")) {
        return { exitCode: 0, stdout: "7777", stderr: "" };
      }
      if (cmd === "sh" && args[0] === "-c" && args[1]?.startsWith("cat /proc/")) {
    ...
    const snap = await provider.getActive();
    expect(snap).toEqual({ name: "firefox", windowTitle: null, processId: 7777 });
    await provider.stop();
  });
  ...
  it("stop() clears interval and disconnects bus", async () => {
    ...
    const bus: LinuxDbusBus = {
      async getProxyObject() {
        return { getInterface: () => ({ Eval: async () => JSON.stringify({}) }) };
      },
      disconnect,
    };
    const executor = makeExecutor(() => ({ exitCode: 0, stdout: "", stderr: "" }));
