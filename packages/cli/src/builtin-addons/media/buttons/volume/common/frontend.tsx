...
const VolumeButtonFrontend = ({ variant, gesture }: VolumeButtonFrontendProps) => {
  ...
  useEffect(() => {
    ...
    timerRef.current = setTimeout(() => setShowProgress(false), PROGRESS_VISIBLE_MS);
    return () => {
      if (timerRef.current) clearTimeout(timerRef.current);
    };
  }, [gesture]);
    ...
    <div className="flex h-full w-full flex-col items-center justify-center gap-1 relative">
      ...
      {showProgress && (
        <div className="absolute bottom-0 left-0 right-0 px-1 pb-1">
          <ProgressBar value={volume} bgColor="bg-primary" bgColorAlt="bg-primary/30" />
        </div>
      )}
    </div>
  );
