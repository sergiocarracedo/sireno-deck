...
import { loadDeviceConfig, saveDeviceConfig } from "./device-config.ts";
...
describe("device-config", () => {
  ...
  it("atomic write doesn't leave .tmp behind on success", () => {
    saveDeviceConfig({ xdgConfigHome: dir, config: { serial: "X", path: "/p", model: "M" } });
    expect(existsSync(join(dir, "sireno-deck-2/device.json.tmp"))).toBe(false);
  });

  it("corrupt JSON returns null", () => {
    const path = join(dir, "sireno-deck-2/device.json");
    require("node:fs").mkdirSync(join(dir, "sireno-deck-2"), { recursive: true });
