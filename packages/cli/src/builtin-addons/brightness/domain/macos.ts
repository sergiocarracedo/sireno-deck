...
export const isMacOS = (platform: NodeJS.Platform): boolean => platform === "darwin";

export const setBrightnessMacOS = async (
  config: BrightnessButtonConfig,
  exec: (cmd: string, args: string[]) => Promise<{ exitCode: number; stderr: string }>,
): Promise<void> => {
  ...
  if (config.action === "down") {
    await exec("osascript", ["-e", `tell application "System Events" to key code 145`]);
    return;
  }
  await exec("osascript", ["-e", `tell application "System Events" to key code 144`]);
};

export const buildMacOSCommand = (config: BrightnessButtonConfig): readonly string[] => {
  if (config.action === "set" && config.value !== undefined) {
...
export const formatCommand = (cmd: readonly string[]): string => cmd.map(escape).join(" ");
