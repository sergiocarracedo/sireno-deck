...
export const createLinuxClipboardProvider = (
  ...
  const writeText = async (text: string): Promise<void> => {
    ...
    const isWayland = waylandDisplay !== undefined && waylandDisplay.length > 0 && waylandDisplay !== "0";
    if (isWayland) {
      const r = await executor.run("sh", ["-c", `${cmd} | wl-copy`]);
      if (r.exitCode === 0) return;
      logger.warn({ stderr: r.stderr }, "clipboard: wl-copy failed");
    }
    const r1 = await executor.run("sh", ["-c", `${cmd} | xclip -selection clipboard`]);
    if (r1.exitCode === 0) return;
    const r2 = await executor.run("sh", ["-c", `${cmd} | xsel --clipboard --input`]);
    if (r2.exitCode !== 0) {
