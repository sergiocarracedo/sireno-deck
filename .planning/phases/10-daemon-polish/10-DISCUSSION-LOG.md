...
- (A) `sireno-deck.token` in `$XDG_RUNTIME_DIR`, 0600, 32 random bytes base64url (recommended).
...
- (A) Naming convention `sireno-deck-addon-*` (recommended).
- (B) Manifest keyword `sireno-deck-addon`.
