...
const formatDateParts = (
  config: { locale?: string; time_zone?: string },
  date: Date,
) => {
...
const DateButtonFrontend: AddonFrontendButton = ({ config }) => {
  const now = useNow(INTERVAL_MS);
  const { day, month, weekday } = formatDateParts(
    config as { locale?: string; time_zone?: string },
    now,
  );
  return (
