...
const fetchWeatherMock = (fetchMod as unknown as { fetchWeather: ReturnType<typeof vi.fn> })
  .fetchWeather;
...
describe("weather backend", () => {
  ...
  it("returns unavailable snapshot when fetchWeather throws", async () => {
    ...
    backend.methods?.["registerCity"]?.("btn-1", { latitude: 0, longitude: 0 }, "metric");
    ...
    expect(Object.keys(result.byCity)).toHaveLength(1);
    const entryResult = Object.values(result.byCity)[0] as Record<string, unknown>;
