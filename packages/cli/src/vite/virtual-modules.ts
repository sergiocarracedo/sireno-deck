...
export const __SIRENO_ADDON_CLEANUP__ = (addonName: string, cleanup: () => void): void => {
...
export const buildAddonsImports = (addons: ReadonlyArray<SirenoVitePluginAddon>): string => {
  const lines: string[] = [];
...
export const buildAddonsRegistryModule = (addons: ReadonlyArray<SirenoVitePluginAddon>): string => {
  const lines: string[] = [];
  for (const addon of addons) {
    if (addon.frontend === undefined) continue;
    if (addon.buttonTypes === undefined) continue;
    const modName = sanitizeIdentifier(addon.name) + "_manifest";
    lines.push(`import * as ${modName} from ${JSON.stringify(addon.frontend.main)};`);
  }
