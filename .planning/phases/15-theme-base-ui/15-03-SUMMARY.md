...
- **Task 01:** Imported `DomThemeUiPresentationProvider` from `sireno-deck-2/cli` in frontend `App.tsx`, wrapping `<Deck>` component. Provider value is `{}` (both themes export empty `ui`; override mechanism in base components checks `themeUi?.text` which is false, falling through to base implementation).
- **Tasks 02-04:** Verified all 6 addon frontends import from `@sireno-deck-2/cli` (date-time, media-player, weather, system-status, value-display, core-buttons). Media Surface components also resolved correctly — no stale paths to deleted theme folders.
